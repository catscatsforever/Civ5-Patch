...
int CvMinorCivAI::GetFriendshipChangePerTurnTimes100(PlayerTypes ePlayer)
{
	...
	if (GET_TEAM(kPlayer.getTeam()).isHasMet(GetPlayer()->getTeam()))
	{
		...
		int iBaseFriendispTimes100 = GetBaseFriendshipWithMajorTimes100(ePlayer);
		if (pEspionage->IsAnySurveillanceEstablished(GetPlayer()->GetID()))
		{
			iShift += min(kPlayer.GetPlayerPolicies()->GetNumericModifier(POLICYMOD_SPY_INFLUENCE), max((90 * 100 - iBaseFriendispTimes100) * 100 / GC.getGame().getGameSpeedInfo().getGoldGiftMod() - (iChangeThisTurn + iShift), 0));
		}
#endif
...
bool CvMinorCivAI::DoMajorCivEraChange(PlayerTypes ePlayer, EraTypes eNewEra)
{
	...
	if(eTrait == MINOR_CIV_TRAIT_MARITIME)
	{
...
#endif
		{
			...
			iNewFood = GetAlliesCapitalFoodBonus(ePlayer, eNewEra);
#else
...
#ifdef CHANGE_FOOD_PROD_MINORS_SCALE
			iNewFood = GetAlliesOtherCityFoodBonus(ePlayer, eNewEra);
#else
...
#ifdef CHANGE_FOOD_PROD_MINORS_SCALE
			iNewProduction = GetAlliesCapitalProductionBonus(ePlayer, eNewEra);
#else
...
#ifdef CHANGE_FOOD_PROD_MINORS_SCALE
			iNewProduction = GetAlliesOtherCityProductionBonus(ePlayer, eNewEra);
#else
