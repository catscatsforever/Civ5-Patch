...
void CvCityStrategyAI::ChooseProduction(bool bUseAsyncRandom, BuildingTypes eIgnoreBldg /* = NO_BUILDING */, UnitTypes eIgnoreUnit/*  = NO_UNIT */)
{
	...
	for(iBldgLoop = 0; iBldgLoop < GC.GetGameBuildings()->GetNumBuildings(); iBldgLoop++)
	{
		...
		if(iBldgLoop != eIgnoreBldg && m_pCity->canConstruct(eLoopBuilding))
		{
			...
			if (GC.getGame().isOption("GAMEOPTION_AI_TWEAKS") && isWorldWonderClass(pkBuildingInfo->GetBuildingClassInfo()))
			{
				iTempWeight = 0;
			}
#endif
	...
	if(!GetCity()->IsPuppet())
	{
		...
		{
#ifdef NQM_AI_GIMP_NO_WORLD_WONDERS
			if (GC.getGame().isOption("GAMEOPTION_AI_TWEAKS") && isWorldProject((ProjectTypes)iProjectLoop))
				continue;
#endif
